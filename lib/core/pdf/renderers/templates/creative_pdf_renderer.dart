...
class CreativePdfRenderer implements PdfRenderer {
  ...
  Future<pw.Document> render(ResumeModel resume) async {
    ...
    if (resume.personalInfo?.phone?.trim().isNotEmpty == true) {
      contactLines.add(pw.Text(resume.personalInfo!.phone!.trim(), style: const pw.TextStyle(fontSize: 10)));
    }
    if (resume.personalInfo?.email?.trim().isNotEmpty == true) {
      contactLines.add(pw.Text(resume.personalInfo!.email!.trim(), style: const pw.TextStyle(fontSize: 10)));
    }
    if (resume.personalInfo?.linkedIn?.trim().isNotEmpty == true) {
      contactLines.add(pw.Text(resume.personalInfo!.linkedIn!.trim(), style: const pw.TextStyle(fontSize: 10)));
    }
    if (resume.personalInfo?.github?.trim().isNotEmpty == true) {
      contactLines.add(pw.Text(resume.personalInfo!.github!.trim(), style: const pw.TextStyle(fontSize: 10)));
    }
    if (resume.personalInfo?.portfolioWebsite?.trim().isNotEmpty == true) {
      contactLines.add(pw.Text(resume.personalInfo!.portfolioWebsite!.trim(), style: const pw.TextStyle(fontSize: 10)));
    }
