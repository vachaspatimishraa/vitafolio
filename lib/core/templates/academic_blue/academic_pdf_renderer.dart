...
class AcademicPdfRenderer extends ResumeTemplateRenderer {
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      pw.Row(
                        ...
                        children: [
                          pw.Text(
                            '${_formatDate(edu.startDate)} - ${_formatDate(edu.endDate)}: ${edu.degree ?? ""}',
                            style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: _academicBlue),
                          ),
                          if (edu.grade?.isNotEmpty == true)
                            pw.Text('GPA: ${edu.grade}', style: const pw.TextStyle(fontSize: 9)),
                        ],
                      ),
                      pw.Text(edu.school ?? '', style: pw.TextStyle(fontSize: 9, fontStyle: pw.FontStyle.italic)),
                      if (edu.fieldOfStudy?.isNotEmpty == true)
                        pw.Text('Field of Study: ${edu.fieldOfStudy}', style: const pw.TextStyle(fontSize: 9)),
                    ],
                  ),
                )),
            pw.SizedBox(height: 12),
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      pw.Row(
                        ...
                        children: [
                          pw.Text(
                            '${_formatDate(exp.startDate)} - ${exp.isCurrentlyWorking == true ? "Present" : _formatDate(exp.endDate)}: ${exp.position ?? ""}',
                            style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: _academicBlue),
                          ),
                          pw.Text(exp.location ?? '', style: const pw.TextStyle(fontSize: 9, color: PdfColors.grey700)),
                        ],
                      ),
                      pw.Text(exp.company ?? '', style: pw.TextStyle(fontSize: 9, fontStyle: pw.FontStyle.italic)),
          ...
          if (resumeData.projects.isNotEmpty) ...[
            ...
            ...resumeData.projects.map((proj) => pw.Container(
                  ...
                  child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
                    children: [
                      pw.Text(proj.projectName ?? '', style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: _academicBlue)),
                      if (proj.technologies?.isNotEmpty == true)
                        pw.Text('Technologies: ${proj.technologies!}', style: pw.TextStyle(fontSize: 9, fontStyle: pw.FontStyle.italic)),
  ...
  pw.Widget _buildHeader(WorkflowState resumeData) {
    ...
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(info.fullName ?? 'Untitled', style: pw.TextStyle(fontSize: 24, fontWeight: pw.FontWeight.bold, color: _academicBlue)),
  ...
  pw.Widget _buildSectionTitle(String title) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(title, style: pw.TextStyle(fontSize: 13, fontWeight: pw.FontWeight.bold, color: _academicBlue)),
