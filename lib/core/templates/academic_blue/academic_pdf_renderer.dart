...
class AcademicPdfRenderer extends ResumeTemplateRenderer {
        ...
        ...validEdu.map((edu) => pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  pw.Text(edu.school?.trim() ?? '', style: pw.TextStyle(fontSize: 9, fontStyle: pw.FontStyle.italic)),
                  if (edu.fieldOfStudy?.trim().isNotEmpty == true)
                    pw.Text('Field of Study: ${edu.fieldOfStudy!.trim()}', style: const pw.TextStyle(fontSize: 9)),
                ],
              ),
            )),
        pw.SizedBox(height: 12),
        ...
        ...validExp.map((exp) => pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  pw.Text(exp.company?.trim() ?? '', style: pw.TextStyle(fontSize: 9, fontStyle: pw.FontStyle.italic)),
      ...
      widgets.addAll([
        ...
        ...validProj.map((proj) => pw.Container(
              ...
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  pw.Text(proj.projectName?.trim() ?? '', style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: _academicBlue)),
                  if (proj.technologies?.trim().isNotEmpty == true)
                    pw.Text('Technologies: ${proj.technologies!.trim()}', style: pw.TextStyle(fontSize: 9, fontStyle: pw.FontStyle.italic)),
  ...
  pw.Widget _buildHeader(WorkflowState resumeData) {
    ...
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(info.fullName?.trim().isNotEmpty == true ? info.fullName!.trim() : 'Untitled', style: pw.TextStyle(fontSize: 24, fontWeight: pw.FontWeight.bold, color: _academicBlue)),
